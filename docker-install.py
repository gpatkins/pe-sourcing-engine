    ...
    if shutil.which("docker"):
        ...
        if install.lower() == 'y':
            try:
                ...
                is_fedora = os.path.exists('/etc/fedora-release') or os.path.exists('/etc/redhat-release')

                if is_fedora:
                    print("Detected Fedora/RHEL - using native package manager...")
                    # Use Fedora's native Docker packages (more reliable)
                    run_command("sudo dnf -y install docker docker-compose-plugin")
                else:
                    # Use Docker's install script for other distros
                    print("Using Docker's official install script...")
                ...
                # Fallback: try native packages
                try:
                    run_command("sudo dnf -y install docker docker-compose-plugin", ignore_errors=False)
                    run_command("sudo systemctl start docker")
                    run_command("sudo systemctl enable docker")
                    run_command("sudo usermod -aG docker $USER", ignore_errors=True)
                    print("✅ Docker installed via fallback method.")
                    return True
                except:
                    print("❌ Fallback also failed.")
                    print("Please install Docker manually:")
                    print("  sudo dnf install docker docker-compose-plugin")
...
def create_env_file(config):
    """Create .env file with ONLY database and auth configuration.
    API keys are stored separately in config/secrets.env to prevent
    Docker Compose from setting empty environment variables."""
    
    env_content = f"""# PE Sourcing Engine v5.5 - Environment Variables
# Generated by installer
# NOTE: API keys are in config/secrets.env (not here)
...
"""
    ...
    print("✅ .env file created (DB + Auth only).")

def create_secrets_env(config):
    """Create config/secrets.env with API keys and other secrets.
    This file is loaded by Python code at runtime, not by Docker Compose."""
    
    secrets_env_content = f"""# PE Sourcing Engine v5.5 - API Keys & Secrets
# Generated by installer on {subprocess.check_output(['date'], text=True).strip()}
# Can be updated via Admin dashboard or manually edited
# Loaded by Python code at runtime (not by Docker Compose)
...
# Google Places API Key (for company discovery)
GOOGLE_PLACES_API_KEY={config.get('google_places_key', '')}

# Google Gemini API Key (for AI classification and owner detection)
GEMINI_API_KEY={config.get('gemini_key', '')}
GOOGLE_GEMINI_API_KEY={config.get('gemini_key', '')}

# Serper.dev API Key (for news and risk analysis)
SERPER_API_KEY={config.get('serper_key', '')}

# Optional: AI Server URL
AI_SERVER_URL={config.get('ai_server_url', '')}

# Metabase Analytics URL
METABASE_URL=http://metabase:3000
"""

    with open("config/secrets.env", "w") as f:
        f.write(secrets_env_content)

    print("✅ config/secrets.env created (API keys + DB connection).")
...
def main():
    ...
    try:
        ...
        print("  Docker Guide:  docker-README.md")
