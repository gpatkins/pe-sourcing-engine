...
def check_docker():
    ...
    if not run_command("docker compose version", ignore_errors=True):
        print("❌ Docker Compose v2 not found. Please install docker-compose-plugin.")
        sys.exit(1)
...
def create_env_file():
    ...
    existing = {}
    if env_path.exists():
        print("\n📄 Existing .env found. Loading current values...")
        with open(env_path, 'r') as f:
            for line in f:
                line = line.strip()
                if line and not line.startswith('#') and '=' in line:
                    key, value = line.split('=', 1)
                    # Don't load "None" as a valid value
    ...
    print("\n--- 🔑 API Keys (press Enter to skip - add later via Admin Dashboard) ---")
    ...
    # Build env content - empty strings for skipped keys, not "None"
...
# API Keys (leave empty to configure via Admin Dashboard after install)
GOOGLE_PLACES_API_KEY={google_places if google_places else ''}
GEMINI_API_KEY={gemini if gemini else ''}
SERPER_API_KEY={serper if serper else ''}
"""
...
def main():
    ...
    if proceed.lower() != 'y':
        print("\n✅ Configuration saved. Run these commands when ready:")
